...
public class MjestoController : ControllerBase, ICustomController<int, Mjesto>
{
  ...
  {
    var query = new MjestoCountQuery(filter);
  ...
  public async Task<List<Mjesto>> GetAll([FromQuery] LoadParams loadParams) 
  {
  ...
  {
    var mjesto = await mjestoQueryHandler.Handle(new MjestoQuery(id));
  ...
  public async Task<IActionResult> Create(Mjesto model)
  {
    ...
    var addedItem = await mjestoQueryHandler.Handle(new MjestoQuery(id));
